...
DEF_CVAR(boner, "how many inches is it", 1, CON_ARCHIVE)

struct con_var *vol = 0;
static bool do_load(ifacefactory enginef, ifacefactory serverf) {
	...
	*p   = (void *)&nop_p_v;		  // OnEdictFreed
	...
	vol = con_findvar("volume");
	con_msg("%p\n", vol);
	con_setvarf(vol, 0.05);
	con_msg("%f\n", con_getvarf(vol));
	con_warn("blalsack tesitcle\n");
	...
	if_hot (pluginhandler) {
		...
		if(GAMETYPE_MATCHES(OE)) {
			cmd_plugin_load->cb_v1 = &hook_plugin_load_cb_oe;
			cmd_plugin_unload->cb_v1 = &hook_plugin_unload_cb_oe;
		} else {
